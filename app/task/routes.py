...
def list_tasks():
    tasks = [
        {"id": t.id, "name": t.name, "description": t.description, "priority": t.priority, "completed": t.completed}
...
def create_task():
    ...
    return jsonify({
        "success": True,
        "task": {"id": task.id, "name": task.name, "description": task.description, "priority": task.priority, "completed": task.completed},
    })
